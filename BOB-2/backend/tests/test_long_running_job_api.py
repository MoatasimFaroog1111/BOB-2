...
def test_active_heavy_routes_use_one_queued_replacement_each():
    active_routes = [
        route
    ...
    for relative_path in _REPLACED_PATHS:
        ...
        matching = [route for route in active_routes if route.path == absolute_path and "POST" in route.methods]
        assert len(matching) == 1, absolute_path
        assert matching[0].endpoint.__module__ == "app.api.v1.long_running_jobs"


def test_large_upload_returns_queued_response_under_one_second(client, auth_headers, db, monkeypatch):
    ...
    monkeypatch.setattr("app.api.v1.long_running_jobs.persist_validated_uploads", fake_persist)
    monkeypatch.setattr("app.api.v1.long_running_jobs.enqueue_background_job", fake_enqueue)
    ...
    status_response = client.get(f"/api/v1/jobs/{body['job_id']}", headers=auth_headers)
...
def test_job_status_cannot_cross_tenant_boundary(client, auth_headers, db):
    ...
    db.add(JobRun(job_id="other-tenant-job", organization_id=2, kind="document_ocr", status="completed", progress=100))
    db.commit()

    response = client.get("/api/v1/jobs/other-tenant-job", headers=auth_headers)
    assert response.status_code == 404


def test_sync_rollback_flag_invokes_hardened_handler(client, auth_headers, monkeypatch):
    async def fake_hardened(*, statement, date_from, date_to):
        return {"status": "success", "mode": "synchronous"}

    monkeypatch.setenv("ASYNC_LONG_RUNNING_JOBS_ENABLED", "false")
    monkeypatch.setattr("app.api.v1.long_running_jobs.hardened_parse_bank_statement", fake_hardened)
    response = client.post(
        "/api/v1/erp/bank-statement-parse",
        files={
            "statement": (
