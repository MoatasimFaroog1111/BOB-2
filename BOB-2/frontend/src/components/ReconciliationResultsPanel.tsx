...
function cleanText(value?: string) { return (value || "").toLowerCase().replace(/[أإآ]/g, "ا").replace(/ى/g, "ي").replace(/ة/g, "ه"); }

function getSelectedBankAccountFromPage() {
  if (typeof document === "undefined") return "";
  const selectedOptions = Array.from(document.querySelectorAll("select option:checked")) as HTMLOptionElement[];
  const option = selectedOptions.find((opt) => {
    const text = (opt.textContent || "").trim();
    return text && !/اختر الحساب|select bank/i.test(text);
  });
...
function suggestCounterAccount(txn: Transaction) {
  ...
  if (txn.ai_suggested_account) return { account: txn.ai_suggested_account, confidence: 82, source: "AI / NLP" };
  if (/رسوم|عموله|charge|fee|commission|bank charge/.test(text)) return { account: "مصروفات بنكية", confidence: 74, source: "NLP keyword" };
  if (/راتب|رواتب|salary|payroll|wps/.test(text)) return { account: "مصروفات الرواتب والأجور", confidence: 72, source: "NLP keyword" };
  if (/مورد|vendor|supplier|فاتوره|bill|purchase|سداد|sadad/.test(text)) return { account: "الموردون / مصروفات تحت التسوية", confidence: 68, source: "Semantic hint" };
  if (/عميل|customer|client|تحصيل|ايراد|revenue|receipt|deposit|ايداع/.test(text)) return { account: "العملاء / إيرادات تحت التسوية", confidence: 68, source: "Semantic hint" };
  if (/ضريبه|vat|tax|زكاه|زكاة/.test(text)) return { account: "ضرائب ورسوم حكومية", confidence: 66, source: "NLP keyword" };
  if (/ايجار|rent|lease/.test(text)) return { account: "مصروفات الإيجار", confidence: 64, source: "NLP keyword" };
...
function buildJournalLines(txn: Transaction, bankAccountLabel: string) {
  ...
  if (isDeposit) {
    return { type: "deposit", amount, suggestion, lines: [ { account: bank, debit: amount, credit: 0, memo }, { account: suggestion.account, debit: 0, credit: amount, memo } ] };
  }
  return { type: "payment", amount, suggestion, lines: [ { account: suggestion.account, debit: amount, credit: 0, memo }, { account: bank, debit: 0, credit: amount, memo } ] };
}
...
function SuggestedEntryCard({ txn, index, isAr, bankAccountLabel }: { txn: Transaction; index: number; isAr: boolean; bankAccountLabel: string }) {
  ...
  return <div className="rounded-xl border border-cyan-500/25 bg-cyan-500/5 p-3 space-y-3"><div className="flex items-start justify-between gap-3 flex-wrap"><div><p className="font-bold text-cyan-300">🧾 {isAr ? "قيد مقترح للعملية" : "Suggested journal entry"} #{index + 1}</p><p className="text-[10px] text-white/55 mt-1">{txn.date} — {txn.main_description || txn.description}</p></div><div className="text-end"><p className="font-mono text-amber-300 font-bold">{fmt(draft.amount)} SAR</p><p className="text-[9px] text-white/45">{draft.type === "deposit" ? (isAr ? "إيداع" : "Deposit") : (isAr ? "تحويل صادر / صرف" : "Payment")}</p></div></div><div className="rounded-lg border border-white/10 overflow-hidden"><table className="w-full text-[10px]"><thead className="bg-white/5 text-white/60"><tr><th className="p-2 text-right">{isAr ? "الحساب" : "Account"}</th><th className="p-2 text-center">{isAr ? "مدين" : "Debit"}</th><th className="p-2 text-center">{isAr ? "دائن" : "Credit"}</th></tr></thead><tbody className="divide-y divide-white/5">{draft.lines.map((line, idx) => <tr key={idx}><td className="p-2 text-white font-semibold">{line.account}</td><td className="p-2 text-center font-mono text-emerald-300">{line.debit ? fmt(line.debit) : "—"}</td><td className="p-2 text-center font-mono text-rose-300">{line.credit ? fmt(line.credit) : "—"}</td></tr>)}</tbody></table></div><div className="rounded-lg bg-black/20 border border-white/10 p-2 text-[10px] text-white/60">{isAr ? "الحساب المقابل مقترح بطريقة مطابقة دلالية بالوصف NLP / Semantic Matching. لا يتم الترحيل إلا بعد الاعتماد." : "Counter account suggested using NLP / Semantic Matching. Nothing is posted until approval."}<span className="ms-2 text-cyan-300 font-mono">{draft.suggestion.confidence}%</span></div></div>;
}
...
export default function ReconciliationResultsPanel({ result, isAr, bankAccountLabel = "" }: { result: ReconciliationResult; isAr: boolean; bankAccountLabel?: string }) {
  ...
  const resolvedBankAccountLabel = bankAccountLabel || getSelectedBankAccountFromPage() || (isAr ? "الحساب البنكي المحدد في شاشة المطابقة" : "Selected bank account from matching screen");
  ...
  return <div className="space-y-3">
    ...
    <div className={`${wideMode ? "fixed inset-3 z-[9999] bg-[#050505] shadow-2xl flex flex-col" : "bg-black/25"} rounded-2xl border ${current.border} overflow-hidden`}>
      ...
      <div className={`${wideMode ? "flex-1 min-h-0 overflow-auto p-4" : "p-3 max-h-[520px] overflow-auto"} space-y-3`}>
        ...
        {activeView === "statement_only" && <div className="space-y-3">{result.statement_only.length === 0 ? <p className="text-white/40">{isAr ? "لا توجد عمليات مسجلة في كشف البنك فقط." : "No bank-statement-only rows."}</p> : <>{!showJournalDrafts && <div className="space-y-2">{result.statement_only.map((txn, idx) => <TxnCard key={idx} txn={txn} label={isAr ? "كشف البنك فقط" : "Bank statement only"} isAr={isAr} />)}</div>}{showJournalDrafts && <div className={`${wideMode ? "grid xl:grid-cols-2 gap-3" : "space-y-3"}`}><div className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-3 text-[11px] text-white/70 xl:col-span-2"><p className="font-bold text-cyan-300 mb-1">🧠 {isAr ? "اقتراح القيود بطريقة NLP / Semantic Matching" : "Journal suggestions using NLP / Semantic Matching"}</p><p>{isAr ? "إذا كانت العملية إيداعًا يظهر الحساب البنكي المحدد في شاشة المطابقة مدينًا. وإذا كانت تحويلًا صادرًا أو صرفًا يظهر نفس الحساب دائنًا. الحساب المقابل مقترح من وصف العملية وبيانات الكشف، ويحتاج اعتمادًا قبل التسجيل." : "Deposits debit the bank account selected in matching. Outgoing transfers/payments credit the same account. Counter account is suggested from transaction description and requires approval before posting."}</p><p className="mt-2 text-cyan-300 font-bold">{isAr ? "الحساب البنكي المستخدم:" : "Bank account used:"} {resolvedBankAccountLabel}</p></div>{result.statement_only.map((txn, idx) => <SuggestedEntryCard key={idx} txn={txn} index={idx} isAr={isAr} bankAccountLabel={resolvedBankAccountLabel} />)}</div>}</>}</div>}
